...
/* Tokenizer: single-quoted strings become a single token */
...
ParsedQuery Parser::parse(const std::string &rawSql) {
    ...
    if (first == "CREATE") {
        ...
        if (pos + 2 < tokens.size() &&
            toUpper(tokens[pos])   == "IF" &&
            toUpper(tokens[pos+1]) == "NOT" &&
            toUpper(tokens[pos+2]) == "EXISTS") {
            q.ifNotExists = true;
            pos += 3;
        }
        ...
        while (pos < tokens.size() && tokens[pos] != ")") {
            ...
            // Skip optional length specifier e.g. VARCHAR(255)
            if (pos < tokens.size() && tokens[pos] == "(") {
                ++pos;
    ...
    if (first == "SELECT") {
        ...
        if (q.where.present) {
            ...
            if (dot != std::string::npos)
                q.where.column = q.where.column.substr(dot + 1);
        }

        // ORDER BY
        if (pos + 1 < tokens.size() &&
            toUpper(tokens[pos])   == "ORDER" &&
